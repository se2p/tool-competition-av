...
def generate(ctx, executor, beamng_home, beamng_user,
    ...
    # Setup model executor by dynamically loading it
    model_config = ModelsConfig()
    model_module = importlib.import_module(model_config.module_name)
    the_model_class = getattr(model_module, model_config.executor_class_name)
    ...
    if executor == "mock":
        ...
        the_executor = the_model_class(result_folder, time_budget, map_size,
                                       oob_tolerance=oob_tolerance, max_speed=speed_limit,
                                       beamng_home=beamng_home, beamng_user=beamng_user,
                                       road_visualizer=road_visualizer)
